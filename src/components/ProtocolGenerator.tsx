        ...
        <section className="py-16 md:py-24 lg:py-32 bg-white relative border-t border-zinc-200 border-b">
            <div className="max-w-[1400px] mx-auto px-6 md:px-8 grid md:grid-cols-2 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
                ...
                <div className="space-y-12">
                    <div>
                        <h2 className="text-sm uppercase tracking-[0.2em] text-zinc-500 mb-6 font-semibold">Interactive</h2>
                        <h3 className="text-5xl lg:text-6xl font-heading leading-[1.1] mb-6 text-zinc-950">
                    ...
                    <div className="space-y-6 md:space-y-8 bg-zinc-50 p-6 md:p-10 relative border border-zinc-200">
                        ...
                        <div>
                            <div className="flex justify-between mb-4">
                                <label className="text-xs uppercase tracking-widest text-zinc-500 font-semibold">Training Intensity (1-10)</label>
                                <span className="text-xs font-bold text-accent">{intensity}/10</span>
                            </div>
                            <input type="range" min="1" max="10" value={intensity} onChange={(e) => setIntensity(Number(e.target.value))} className="w-full accent-accent bg-zinc-200 h-[2px] rounded-none appearance-none outline-none cursor-pointer" />
                        </div>

                        <div>
                            <div className="flex justify-between mb-4">
                                <label className="text-xs uppercase tracking-widest text-zinc-500 font-semibold">Sleep Last Night (Hours)</label>
                                <span className="text-xs font-bold text-accent">{sleep}h</span>
                            </div>
                            <input type="range" min="3" max="12" value={sleep} onChange={(e) => setSleep(Number(e.target.value))} className="w-full accent-accent bg-zinc-200 h-[2px] rounded-none appearance-none outline-none cursor-pointer" />
                        </div>

                        <div>
                            <label className="text-xs uppercase tracking-widest text-zinc-500 font-semibold block mb-4">Primary Stimulus Goal</label>
                            <div className="grid grid-cols-2 gap-3">
                                {["CNS Reset", "Tissue Repair", "Inflammation Reduction", "General Maintenance"].map((goal) => (
                                    <button
                                        key={goal}
                                        onClick={() => setFocus(goal)}
                                        className={`py-4 text-[10px] md:text-xs tracking-widest uppercase transition-all border ${focus === goal ? "bg-accent text-white border-accent" : "border-zinc-200 text-zinc-500 hover:border-zinc-400 hover:text-zinc-800"}`}
                                    >
                                        {goal}
                                    </button>
                                ))}
                        ...
                        <button
                            ...
                            disabled={generating}
                            className="w-full py-5 bg-zinc-950 text-white text-xs font-bold uppercase tracking-[0.2em] hover:bg-accent transition-all duration-300 disabled:opacity-50"
                        >
                ...
                <div className="h-full min-h-[400px] md:min-h-[500px] flex items-center justify-center border border-zinc-200 bg-zinc-50 relative p-6 md:p-10 shadow-lg">
                    {!protocol && !generating && (
                        <div className="text-center">
                            ...
                            <p className="text-zinc-400 text-xs tracking-[0.2em] uppercase font-bold">Awaiting Inputs</p>
                        </div>
                    )}

                    {generating && (
                        <div className="text-center w-full">
                            <div className="w-full h-[2px] bg-zinc-200 relative overflow-hidden mb-8">
                                <div className="absolute top-0 left-0 h-full bg-accent animate-[loading_1.5s_infinite_ease-in-out]"></div>
                            </div>
                            <p className="text-zinc-500 text-xs font-bold tracking-[0.2em] uppercase">Calculating precise interventions...</p>
                        </div>
                    )}

                    {protocol && !generating && (
                        <div className="w-full animate-in fade-in slide-in-from-bottom-8 duration-700">
                            <h4 className="text-3xl font-heading mb-10 pb-6 border-b border-zinc-200 text-zinc-950">{protocol.title}</h4>
                            <div className="space-y-10">
                                {protocol.steps.map((step: { phase: string; desc: string }, idx: number) => (
                                    <div key={idx} className="flex gap-6">
                                        ...
                                        <div>
                                            <h5 className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-800 mb-3">{step.phase}</h5>
                                            <p className="text-zinc-600 font-light text-base leading-relaxed">{step.desc}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            <div className="mt-14 pt-8 border-t border-zinc-200">
                                <button className="text-xs uppercase tracking-[0.2em] text-accent font-bold flex items-center gap-3 hover:text-zinc-950 transition-colors">
